...
-- There is missing property address data for some observations, but these observations have ParcelID values that match other observations with the correct property address info
-- Using a self-join to show this and populate those missing property addresses
...
SELECT orig.ParcelID, orig.PropertyAddress, copy.ParcelID, copy.PropertyAddress, ISNULL(orig.PropertyAddress, copy.PropertyAddress)
...
-- Using Charindex to find the location of the delimiter (comma in this case) and Substring to separate based on that location
-- the '-1' and '+1' are used to leave the comma out of the returned values
SELECT
...
-- Should use +2 Next time in CHARINDEX...Hindsight 20/20
UPDATE NashvilleHousing
